      ...
      if (validDomains.length > 0 && !validDomains.includes(subdomain)) {
        ...
        if (closestSubdomain) {
          ...
          if (isLocal) {
            ...
            // Para producción: redirigir a closestSubdomain.restaurantes.cloud (sin puerto)
            const domainParts = hostname.split('.');
            const baseDomain = domainParts.slice(-2).join('.'); // Obtener los últimos 2 (ej: restaurantes.cloud)
